...
def start_game(user_name, gameId, game_number, score, opponent_user_name, opponent_score, server_game_number):
...
def want_to_play(time1, user_name, command, sender_msg):
    ...
    if status_code == 9:
        start_game(user_name1, game_id, game_number1, score1, user_name, score, server_game_number)
...
def opponent_play_cell_game(user_name, opponent_user_name, server_game_number, cell, status_code, score, opponent_score):
    print("Server.opponent_play_cell_game server_game_number:" + server_game_number + " cell:" + str(cell))
    time = datetime.utcnow().strftime('%Y%m%d%H%M%S')
    ...
    print("Server.opponent_play_cell_game sending:" + str_data)
    messages_to_send.append((users[user_name], str.encode(str_data)))


def play_cell_game(time1, user_name, command, sender_msg):
